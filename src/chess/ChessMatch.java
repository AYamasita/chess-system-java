...
public class ChessMatch {
	...
	// retorna uma matriz do tipo ChessPiece
	public ChessPiece[][] getPieces() {
		ChessPiece[][] mat = new ChessPiece[board.getRows()][board.getColumns()];
		for (int i = 0; i < board.getRows(); i++) {
			for (int j = 0; j < board.getColumns(); j++) {
				// downcasting board.piece -> chessPiece
	...
	//desfaz o movimento poque o usuario não pode colocar em check
	private void undoMove(Position source, Position target, Piece capturedPiece) 
	{
		...
		if(capturedPiece != null)
		{
			board.placePiece(capturedPiece, target);//coloca a peça captured no tabuleiro
	...
	private boolean testCheckMate(Color color)
	{
		...
		for(Piece p : pieces)
		{
			...
			for(int i=0;i<board.getRows();i++)
			{
				for(int j=0;j<board.getColumns();j++)
				{
					if (mat[i][j]) //movimento possivel? -> true
					{
						Position source = ((ChessPiece)p).getChessPosition().toPosition();
						Position target = new Position(i,j);
						Piece capturedPiece = makeMove(source, target);
						boolean testCheck = testCheck(color); //capturo o estado.
						undoMove(source, target, capturedPiece);
						if (!testCheck)
						{
							return false;
